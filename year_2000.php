...
?>
<p class="title">Niche associated with infections</p>

<div align="center" class="year">

<form action="#" method="post">
<label>Select decade</label>
<select name="yop" onChange="this.form.submit()">
...
<canvas id="year_2000" style="width:100%;max-width:1200px; padding: 30px;"></canvas>
<?php 
include "connect.php";
...
$ni = mysqli_query($conn, "SELECT distinct niche from niche_infected where niche != '-' order by niche");
while($ni_res = mysqli_fetch_array($ni))
{
	...
	if(strpos($yop, '<') !== false)
	{
	$qry = "SELECT count(DISTINCT Study_ids) as cont FROM data where Niche_Infected LIKE '%$val%' and Year_of_Publication $yop";
	$title = "Upto 2000";
	}
		...
		$qry = "SELECT count(DISTINCT Study_ids) as cont FROM data where Niche_Infected LIKE '%$val%' and Year_of_Publication Between $yop";
	...
	$qry = "SELECT count(DISTINCT Study_ids) as cont FROM data where Niche_Infected LIKE '%$val%'";
    $title = "1972-2022";
