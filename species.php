...
include("connect.php");     
	...
	if($cntt==0)
	{
		...
		$col="PMID_DOI, Year_of_Publication, Risk_Factors_Comorbidities, Type_of_Candidiasis, Niche_Infected, Species_Prevalence, Symptoms_Clinical_Manifestations_Signs";
...
$cntsp = 0;
 ...
 if(isset($_POST['spsname'])){
	 foreach($_POST['spsname'] as $snm ){
...
<p>Select species</p>
  <div>
		...
		<input type="checkbox" name="spsname[]" value="C. albicans" onChange="this.form.submit()" <?php if(in_array("C. albicans", $spsnms))
  {
  echo "checked";
  }?>>
  <label for="spsname"><em>C. albicans</em></label><br/>
  <input type="checkbox" name="spsname[]" value="Non-albicans Candida" onChange="this.form.submit()" <?php if(in_array("Non-albicans Candida", $spsnms))
  {
  echo "checked";
  }?>>
  <label for="spsname">Non-<em>albicans Candida</em></label><br/>
  <?php
	
	
	$dis_sps = mysqli_query($conn, "select distinct species from sps_prevalence where species !='C. albicans' and species !='Unidentified' and species !='Non-albicans Candida' order by species ASC;");
	 foreach($dis_sps as $val){
		 $sps = $val["species"];
		 $arrsps[] = $val["species"];
		?>
	  <input type="checkbox" name="spsname[]" value="<?php echo $sps; ?>" onChange="this.form.submit()" <?php if(in_array("$sps", $spsnms))
  {
  echo "checked";
  }?>>
  <label for="spsname"><em><?php echo $sps; ?></em></label><br/>
	  
	<?php
	 }
  ?>
	<input type="checkbox" name="spsname[]" value="Unidentified" onChange="this.form.submit()" <?php if(in_array("Unidentified", $spsnms))
  {
  echo "checked";
  }?>>
  <label for="spsname">Species unidentified</label><br/>
	</form>
  </div>
	...
	if(isset($spnm)){
		?>
    <h3>Studies reporting <em><?php echo $spnm; ?></em></h3>
...
<tbody>
	...
	if(isset($_SESSION["spsnms"])){
		
		
		$cnttt = 0;
		$query = "select * from data where"; 
		
	foreach($spsnms as $sp){
		
		
		
		if($cnttt == 0){
			$query = $query." Species_Prevalence LIKE '%$sp%'";
			
		}
		else{
			$query = $query." OR Species_Prevalence LIKE '%$sp%'";
...
$('#sortTable').DataTable();
		...
		<input type="hidden" value="<?php echo $dnld;?>" name="down_sps">
	    </form>
