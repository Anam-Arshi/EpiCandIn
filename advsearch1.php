...
<head>
<meta charset="utf-8">
<title>Advanced search- Records</title>
...
<?php
$dnld="";
include("connect.php");     #$qury = "[Species_Prevalence]{C. albicans} AND [Niche_Infected]{Oropharynx}";
...
//echo($qury);
	...
	$replacement="%'";
	$qury=str_replace($lookFor, $replacement, $qury);
	$lookFor="[Species_Prevalence]";
	$replacement="Species_Prevalence LIKE ";
	$qury=str_replace($lookFor, $replacement, $qury);
	$lookFor="[Niche_Infected]";
	$replacement="Niche_Infected LIKE ";
	$qury=str_replace($lookFor, $replacement, $qury);
	$lookFor="[Antifungal_Susceptibility_Profile]";
	$replacement="Antifungal_Susceptibility_Profile LIKE ";
	$qury=str_replace($lookFor, $replacement, $qury);
	$lookFor="[Type_of_Candidiasis]";
	$replacement="Type_of_Candidiasis LIKE ";
	$qury=str_replace($lookFor, $replacement, $qury);
	$lookFor="[Study_Design]";
	$replacement="Study_Design LIKE ";
	$qury=str_replace($lookFor, $replacement, $qury); 
	$lookFor="[Year_of_Publication]";
	$replacement="Year_of_Publication LIKE ";
	$qury=str_replace($lookFor, $replacement, $qury); 
	
    
	$querw="SELECT * FROM data  WHERE ".$qury."";
	

	
	$res = mysqli_query($conn, $querw); 
...
<label for="col19">Location</label><br/>
 ...
 <input type="checkbox" id="col15" name="col[]" value="Method_of_Antifungal_Susceptibility_Testing" onChange="this.form.submit()" <?php if (in_array("Method_of_Antifungal_Susceptibility_Testing", $col1))
  ...
  <label for="col15">Antifungal susceptibility test</label><br/>
   <br/><br/>	
 	<input type="hidden" name="qry" value="<?php echo $qury; ?>">
	</form>
</div>
	...
	?>
	<p style="margin-left: 20px; display: block;"><b>
	<?php
	$qr = str_replace("[", "", $qur);
	$qr = str_replace("]", "-", $qr);
	$qr = str_replace("{", "(", $qr);
...
$('#sortTable').DataTable();
		...
		<input type="hidden" value="<?php echo $dnld;?>" name="down_advs"></input>
	    </form>
...
<?php
		...
		echo "<h4>&nbsp &nbsp No results for this query</h4>";
	}
	?>
	</div>
	<?php
