...
include("connect.php");     $cntt=0;
	...
	if($cntt==0)
	{
		...
		$col="PMID_DOI, Year_of_Publication, Risk_Factors_Comorbidities, Type_of_Candidiasis, Niche_Infected, Name_of_Drug, Antifungal_Susceptibility_Profile";
...
$cntdr = 0;
 ...
 if(isset($_POST['drgname'])){
	 foreach($_POST['drgname'] as $dnm ){
...
<p>Select Drugs</p>
  ...
  <?php
	...
	$dis_drg = mysqli_query($conn, "select distinct Drug from asp where Drug !='Highly sensitive to Clotrimazole, Fluconazole, Itraconazole' AND Drug NOT LIKE '%+%' order by Drug ASC");
	 foreach($dis_drg as $val){
		 $drg = $val["Drug"];
		 
		?>
	  <input type="checkbox" name="drgname[]" value="<?php echo $drg; ?>" onChange="this.form.submit()" <?php if(in_array("$drg", $drgnms))
  {
  echo "checked";
  }?>>
  <label for="drgname"><?php echo $drg; ?></label><br/>
  ...
  ?>
    ...
    if(isset($drnm)){
		?>
    <h3>Studies with drugs <?php echo $drnm; ?></h3>
		<?php
		}else{
		?>
...
?>
	...
	if(isset($_SESSION["drgnms"])){
		
		
		$cnttt = 0;
		$query = "select * from data where"; 
		
	foreach($drgnms as $dr){
		
		
		
		if($cnttt == 0){
			$query = $query." Antifungal_Susceptibility_Profile LIKE '%$dr%'";
			
		}
		else{
			$query = $query." OR Antifungal_Susceptibility_Profile LIKE '%$dr%'";
...
$('#sortTable').DataTable();
		...
		<input type="hidden" value="<?php echo $dnld;?>" name="down_drug">
	    </form>
