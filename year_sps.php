...
while($ni_res = mysqli_fetch_array($ni))
{
	...
	if(strpos($yop, '<') !== false)
	{
	$qry = "SELECT count(DISTINCT Study_ids) as cont FROM data where Species_Prevalence LIKE '%$val%' and Year_of_Publication $yop";
	$title = "Upto 2000";
	}
		...
		$qry = "SELECT count(DISTINCT Study_ids) as cont FROM data where Species_Prevalence LIKE '%$val%' and Year_of_Publication Between $yop";
	...
	if($yop == "default"){
        	$qry = "SELECT count(DISTINCT Study_ids) as cont FROM data where Species_Prevalence LIKE '%$val%'";
    $title = "1972-2022";
