...
<p align='center' style="padding: 14px;">
<?php
	echo $title;
...
?>
  ...
  if($cnt_res > 0)
  {
	...
	$r =  $val["R"];
	 $in = $val["I"];
	$s = $val["S"];
    $dr = "'Resistant', 'Susceptible', 'Intermediate'";
		$per = " '$r', '$s', '$in' ";	
...
var downloadGroup = svg.append('g')
  .attr('transform', 'translate('+ 350 +', 12)'); // Adjust the position as needed
...
downloadGroup.on('click', downloadGraph);
			...
			if(isset($sp))
			{
				...
				foreach($sp as $spp){
					if($spp != 'NACs'){
			         ...
			         echo "<h4 style='padding: 10px; background-color: lightgray;'>Organism: <em>$spp</em></h4>";
			
					}
					else{
						echo '<h4 style="padding: 10px; background-color: lightgray;"><em>Non-alibicans Candida species</em></h4>';
						
					}
      foreach($drug as $drr){
	 		 $val_csv = "activity,$dc[0],$dc[1]".'\n';
		  
			 if($spp != ""){
		
		$qre1 ="select R, I, S from asp where study_ids='$st_id' and Species ='$spp' and Drug ='$drr' and Disease_Category='$dc[0]' and (R !='0' OR I !='0' OR S !='0')";
				 
		$qre2 ="select R, I, S from asp where study_ids='$st_id' and Species ='$spp' and Drug ='$drr' and Disease_Category='$dc[1]' and (R !='0' OR I !='0' OR S !='0')";
				 //echo $qre1;
				 
	 }
		else{
		
		$qre1 ="select R, I, S from asp where study_ids='$st_id' and Drug ='$drr' and Disease_Category='$dc[0]' and (R !='0' OR I !='0' OR S !='0')";
			
		$qre2 ="select R, I, S from asp where study_ids='$st_id' and Drug ='$drr' and Disease_Category='$dc[1]' and (R !='0' OR I !='0' OR S !='0')";
        }
	
	 $res1 = mysqli_query($conn, $qre1);
		  
	$res2 = mysqli_query($conn, $qre2);
	
	   $cnt_res = mysqli_num_rows($res1);

	unset($r1, $in1, $s1, $r2, $s2, $in2);
	foreach($res1 as $val){
	$r1 =  trim($val["R"]);
	 $in1 = trim($val["I"]);
	$s1 = trim($val["S"]);
	...
	$val_csv .= "Susceptible,$s1,$s2".'\n';
	$val_csv .= "Intermediate,$in1,$in2".'\n';
	$val_csv .= "Resistant,$r1,$r2".'\n';

		  
		  
				include("asp_groupBar.php");
