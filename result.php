...
<?php
include("connect.php");     $qry = $_SESSION["qry"];
$qry1 = $_REQUEST["val"];
#echo $qry1;

$cols ="Species_Prevalence LIKE '%$qry%' OR Niche_Infected LIKE '%$qry%' OR Location LIKE '%$qry%'";
$res_stid = mysqli_query($conn, "select distinct $qry1 from data where $cols");
...
<label for="col19">Location</label><br/>
  ...
  <input type="checkbox" id="col22" name="col[]" value="No_of_Candida_Isolates" onChange="this.form.submit()" <?php if (in_array("No_of_Candida_Isolates", $col1))
  {
  echo "checked";
  }?>> 
  <label for="col22">No. of <em>Candida</em> isolates</label>
 ...
 <input type="checkbox" id="col15" name="col[]" value="Method_of_Antifungal_Susceptibility_Testing" onChange="this.form.submit()" <?php if (in_array("Method_of_Antifungal_Susceptibility_Testing", $col1))
  ...
  <br/><br/>	
	...
	<div id="main-sidebar">
    <br>
	<b><?php echo "&nbsp &nbsp Query: ".$qry; ?></b><br>
<?php
...
<!---	<div class="dataset"> -->
	...
	<?php	
	#echo "&nbsp &nbsp $cnt_stid Records";
...
?>
	...
	<?php
		...
		$query = "select * from data where $cols";
			...
			while($row=mysqli_fetch_array($res))
	{
			...
			<tr>
				...
				$pmid = $row["PMID_DOI"];
		        $stid = $row["study_ids"];
				#$tnp = $row["Total_no_of_Patients_Samples"];
				#echo $tnp;
				...
				echo "<a href='study_wise_sps_prev.php?std=$stid' target='_blank1'>".'<i class="fa-solid fa-chart-column" style="padding: 2px; font-size: 25px;"></i>'."</a>";
			...
			else
			{
				...
				<?php
				if($val_asp == 1){
				echo "<a href='asp_graphs.php?std=$stid' target='_blank2'>".'<i class="fa-solid fa-chart-column" style="padding: 2px; font-size: 25px;"></i>'."</a>";
...
$('#sortTable').DataTable();
		...
		<input type="hidden" value="<?php echo $dnld;?>" name="down_result">
	    </form>
