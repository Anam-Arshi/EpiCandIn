...
include("connect.php");     
		...
		$state_name = str_replace('%20', ' ', $state_name);
		#echo "select study_ids from state_city where state_ut = '$state_name'";
		$st = mysqli_query($conn, "select distinct study_ids from state_city where state_ut = '$state_name'");

$cntst = mysqli_num_rows($st);

		

	?>
<h4>Studies from <?php echo($state_name); ?></h4>
...
?>
	        ...
	        $res= mysqli_query($conn, "select * from data where study_ids = '$st_id'");
...
$('#sortTable').DataTable();
		...
		<input type="hidden" value="<?php echo $dnld;?>" name="down_state">
	    </form>
				<?php 
