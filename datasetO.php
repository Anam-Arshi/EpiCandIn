...
$cntt=0;
	...
	if($cntt==0)
	{
		...
		$col="PMID_DOI, Year_of_Publication, Risk_Factors_Comorbidities, Type_of_Candidiasis, Symptoms_Clinical_Manifestations_Signs, Infection_Colonization_Prevalence";
...
<label for="col19">Location</label><br/>
 ...
 <input type="checkbox" id="col15" name="col[]" value="Method_of_Antifungal_Susceptibility_Testing" onChange="this.form.submit()" <?php if (in_array("Method_of_Antifungal_Susceptibility_Testing", $col1))
  ...
  <br/><br/>	
	...
	<div id="main-sidebar">
    <h3>Publications on Candidiasis from India between 1976-2022</h3>
...
echo "&nbsp &nbsp Total $cntres Records";

	?>
	<form action="download.php" method="post" enctype="multipart/form-data">
		
		
		<button type="submit" value="download" title="Download" style="float: right; margin-right: 25px; margin-bottom: 5px;">
...
<!---	<div class="dataset"> -->
	...
	foreach($col1 as $colnm){
		...
		if($colnm == "No_of_Candida_Isolates"){
			echo "<th>No. of <em>Candida</em> isolates</th>";
			if($cntl != 1){
...
?>
			...
			while($row=mysqli_fetch_array($res))
	{
			...
			<tr>
				...
				if($cols == "Type_of_Candidiasis"){
				$res1=mysqli_query($conn, "select type_of_candidiasis from candidiasis where study_ids='$stid'");
		...
		{ 
			if($cnd==0)
			{
				$typ_can = $row1["type_of_candidiasis"];
			}
			else{
				$typ_can = $typ_can.", ".$row1["type_of_candidiasis"];
			}
