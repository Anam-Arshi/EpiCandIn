...
include("connect.php");     
	...
	//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
...
{	
	...
	<table align="center" class="table table-bordered" id="sortTable" width="98%" cellpadding="5" cellspacing="5">
	<thead>
	<tr>
			...
			while($row=mysqli_fetch_array($res))
	{
			...
			<tr>
				...
				if($cols == "PMID_DOI"){
					...
					echo "<td><a href='https://pubmed.ncbi.nlm.nih.gov/$pmid' target='_blank1'>".$pmid."</a></td>";
					$dnld="$dnld\n$pmid";
					}
					else{
						echo "<td><a href='$pmid' target='_blank1'>".$pmid."</a></td>";
						$dnld="$dnld\n$pmid";
					}
				}
				else
				...
				else if($cols == "Demographic_Sex")
				{
				  ...
				  #$demo1 = str_replace(";", "<br>", $demo);
					...
					$chk_sp = mysqli_query($conn, "SELECT EXISTS(SELECT 1 FROM sps_prevalence WHERE Study_ids = '$stid' AND prevalence !='NA' LIMIT 1);");
				...
				echo "<a href='study_wise_sps_prev.php?std=$stid' target='new1'>".'<i class="fa-solid fa-chart-column" style="padding: 2px; font-size: 25px;"></i>'."</a>";
			...
			else
			{
				...
				if($val_asp == 1){
					if($stid != '73'){
				echo "<a href='asp_graphs.php?std=$stid' target='new2'>".'<i class="fa-solid fa-chart-column" style="padding: 2px; font-size: 25px;"></i>'."</a>";
